...
class DocumentoRecibidoController extends Controller
{
    ...
    public function documentosRecibidosTurnar($id)
    {
        ...
        elseif($user->nivel == 6)
        {            
            $listaDepartamentos = Area::where('unidad',$user->id_area)->get();
        }
    ...
    public function documentosRecibidosExport()
    {
        ...
        // REPORTE PARA SUBDIRECTOR - TODOS LOS REGISTROS
        if ($user->nivel == 3) 
        {            
            $documentos = DocumentoRecibido::where('subdireccion_id', $user->id_area)
                ->orderBy('created_at', 'DESC')
        ...
        return Excel::download(new DocumentosRecibidosExport($documentos, $user), 'documentos_recibidos.xlsx');
    }
